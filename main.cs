...
namespace DroneGames
{
    public partial class GamesWin : Form
    {
        ...
        private void goProj()
        {
            ...
            node.projLeft = Mplayer.Left + (Mplayer.Width / 2); 
            node.projHight = Mplayer.Top + (Mplayer.Height / 2); 
            node.launch(this);
        }

        private void clockEvent(object sender, EventArgs e)
        {
            ScoreInfo.Text = $"Score: {scoreCounter}";
            ...
            foreach (Control entityProj in this.Controls)
            {
                ...
                {
                    entityProj.Top += projectilesSpeed;
                }
